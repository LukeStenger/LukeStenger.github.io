# Project Name

Description of your project.

## Custom Change: Spotify Button

A custom change has been implemented to enhance the user experience. A Spotify button has been added, providing a direct link to Luke Stenger's Spotify playlist titled "Luke's Favorites."

### Spotify Playlist - Luke's Favorites

[![Spotify - Luke's Favorites](https://img.shields.io/badge/Spotify-Luke's%20Favorites-1DB954?logo=spotify&logoColor=white&style=flat-square)](https://open.spotify.com/playlist/0fd7vv4hdV1w9hGyorZJ7w?si=1cb5a726d0294fa6)

**Listen to Luke's curated favorites on Spotify!**

### Usage

1. Click on the Spotify button or use the provided badge above.
2. You will be redirected to Luke Stenger's Spotify playlist on the Spotify platform.
3. Enjoy listening to the curated favorites!

### Credits

- Luke Stenger: [Spotify Profile](https://open.spotify.com/user/lstenger7?si=57378f6039e84087)
- Playlist: [Luke's Favorites](https://open.spotify.com/playlist/0fd7vv4hdV1w9hGyorZJ7w?si=1cb5a726d0294fa6)

### Author

Luke Stenger
